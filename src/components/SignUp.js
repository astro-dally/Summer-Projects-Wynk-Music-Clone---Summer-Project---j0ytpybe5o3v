...
function ImageBanner({ src, alt }) {
    return (
        <img
            ...
            src={src}
            alt={alt}
            className="image-css h-60px object-cover absolute inset-0 size-full"
        />
    );
}
...
function SignInForm({ setToken }) {
    ...
    const handleSignUp = async (e) => {
        ...
        if (
            ...
            alert(
                "Password must be at least 6 characters long and include both lower and uppercase letters and symbols"
            );
            return;
        }
    };
